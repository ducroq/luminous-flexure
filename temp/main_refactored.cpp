...
namespace Config {
    ...
    const float ARM_LENGTH = 11.0;        // cm
    const float LED_MAX_LENGTH = 30.0;    // cm
    const float LED_MIN_LENGTH = 8.0;     // cm
...
public:
    ...
    void update(unsigned long current_time, int& angle1, int& angle2) {
        ...
        if (!in_transition && time_in_pattern_sec >= varied_duration) {
            ...
            // Randomly change waveform type for variety
            // 60% sine (smooth), 25% triangle (linear), 15% square (sharp)
            int rand_val = random(100);
            if (rand_val < 60) {
                motion_generator.setWaveformType(0);  // Sine
            } else if (rand_val < 85) {
                motion_generator.setWaveformType(1);  // Triangle
            } else {
                motion_generator.setWaveformType(2);  // Square
            }
